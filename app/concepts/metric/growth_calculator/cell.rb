class Metric::GrowthCalculator::Cell < Cell::Concept
  ...
  def delay_in_days
    [
      { keys: [:verified_sent_invitations, 'avg_delay_in_hours'], convert: -> (val) { val.to_f / 24 } },
      { keys: [:invited_to_registered, 'avg_delay_in_hours'],     convert: -> (val) { val.to_f / 24 } },
      { keys: [:registered_to_verified, 'avg_delay_in_minutes'],  convert: -> (val) { val.to_f / 24 / 60 } }
