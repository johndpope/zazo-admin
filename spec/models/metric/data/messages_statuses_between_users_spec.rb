...
RSpec.describe Metric::Data::MessagesStatusesBetweenUsers, type: :model do
  let(:factor)   { 1 }
  ...
  describe '#generate' do
    ...
    it do
      Benchmark.measure do
        expected = {
          friend_1 => {
            outgoing: { sent: 4 * factor, incomplete: 3 * factor, unviewed: 2 * factor },
            incoming: { sent: 5 * factor, incomplete: 4 * factor, unviewed: 2 * factor }
          },
          friend_2 => {
            outgoing: { sent: 5 * factor, incomplete: 4 * factor, unviewed: 3 * factor },
            incoming: { sent: 6 * factor, incomplete: 5 * factor, unviewed: 2 * factor }
          },
          friend_3 => {
            outgoing: { sent: 0, incomplete: 0, unviewed: 0 },
            incoming: { sent: 0, incomplete: 0, unviewed: 0 }
          },
          total: {
            outgoing: { sent: 9  * factor, incomplete: 7 * factor, unviewed: 5 * factor },
            incoming: { sent: 11 * factor, incomplete: 9 * factor, unviewed: 4 * factor }
          }
        }
